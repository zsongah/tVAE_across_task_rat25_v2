...
class DFINE_runner:
    ...
    def run(self):
        ...
        for epoch in tqdm(range(start_epoch+1,self.config.TRAIN.NUM_UPDATES+1), desc='Training',unit='epoch'):
            ...
            if epoch % self.config.TRAIN.VAL_INTERVAL == 0:
                ...
                self.logger.info(f"reg_loss: {results['loss'][2]:.5f}")
                self.logger.info(f"behv_loss: {results['loss'][3]:.5f}")
        ...
        self._plot_result(results, loss_types = ['total_all', 'recon_loss', 'reg_loss', 'behv_loss'])
    ...
    def train_1_epoch(self, epoch) -> list:
        ...
        self.logger.info(f"reg_loss: {loss_log[-1][2]:.5f}")
        self.logger.info(f"behv_loss: {loss_log[-1][3]:.5f}")
