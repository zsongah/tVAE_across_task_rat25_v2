...
from models.DFINE_modules.LDM import LDM
from models.DFINE_modules.MLP import MLP
from models.DFINE_modules.nn import get_activation_function, get_kernel_initializer_function, compute_mse
...
class DFINE(nn.Module):
    ...
    * Please note that DFINE can perform learning and inference both for continuous data or trial-based data or segmented continuous data. In the case of continuous data,
    ...
    '''
    def __init__(self, config, device, model_type = 'DFINE'):
        ...
        A, C, W_log_diag, R_log_diag, mu_0, Lambda_0 = self._init_ldm_parameters() # 还没有move到设备
    ...
    def _set_dims_and_scales(self):
        ...
        self.dim_x = self.config.MODEL.LATENT_DIM
    ...
    def forward(self, y, mask=None):
        ...
        * Terminology definition:
        ...
        a_hat = a_hat.reshape(num_seq, num_steps, self.dim_a)
        # Run LDM to infer filtered and smoothed dynamic latent factors
        x_pred, x_filter, x_smooth, Lambda_pred, Lambda_filter,Lambda_smooth = self.ldm(a=a_hat, mask=mask, do_smoothing=False)
        ...
        a_filter = (C @ x_filter.unsqueeze(dim = -1)).squeeze(dim = -1)
        a_smooth = (C @ x_smooth.unsqueeze(dim=-1)).squeeze(dim=-1)
        ...
        y_hat = self.decoder(a_hat.reshape(-1, self.dim_a))
        y_pred = self.decoder(a_pred.reshape(-1, self.dim_a))
        y_filter = self.decoder(a_filter.reshape(-1, self.dim_a))
        y_smooth = self.decoder(a_smooth.reshape(-1, self.dim_a))

        y_hat = y_hat.reshape(num_seq, -1, self.dim_y)
        y_pred = y_pred.reshape(num_seq, -1, self.dim_y)
        y_filter = y_filter.reshape(num_seq, -1, self.dim_y)
        y_smooth = y_smooth.reshape(num_seq, -1, self.dim_y)
    ...
    def compute_loss(self, y, model_vars, mask=None, hehv=None):
        '''
        ...
        for _, k in enumerate(self.config.loss.steps_ahead):
            y_pred_k, _, _ = self.get_k_step_ahead_prediction(model_vars, k)
        ...
        if self.config.model.supervise_behv:
            ...
            behv_mse = compute_mse(y_flat=hehv[..., self.config.model.which_behv_dims].reshape(-1, self.dim_behv), 
        ...
        for name, param in self.named_parameters():
            if 'weight' in name:
                reg_loss += self.scale_l2 * torch.norm(param)
    ...
    def get_k_step_ahead_prediction(self, model_vars, k):
        ...
        for i in range(1, k+1):
            if i != k:
                x_pred_k = (A[:, i:-(k-i), ...] @ x_pred_k.unsqueeze(dim=-1)).squeeze(dim=-1)  # [:-1] 不包含最后一个 
            else:
                x_pred_k = (A[:, i:, ...] @ x_pred_k.unsqueeze(dim=-1)).squeeze(dim=-1)  # 拉到底
        ...
        y_pred_k = self.decoder(a_pred_k.reshape(-1, self.dim_a))
